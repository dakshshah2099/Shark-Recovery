...
async def send_whatsapp_message(payload: WhatsAppPayload) -> Dict[str, Any]:
    ...
    if settings.TWILIO_ACCOUNT_SID and settings.TWILIO_AUTH_TOKEN and settings.TWILIO_ACCOUNT_SID.strip() != "":
        try:
            ...
            logger.info(f"Twilio WhatsApp message sent successfully: SID {twilio_sid} to {to_whatsapp}")
        except Exception as e:
            logger.warning(f"Twilio WhatsApp dispatch failed ({str(e)}). Logging to outreach hub buffer.")
            delivery_status = f"twilio_error: {str(e)[:60]}"
