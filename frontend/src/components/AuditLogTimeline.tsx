...
export const AuditLogTimeline: React.FC<AuditLogTimelineProps> = ({ logs }) => {
  ...
  return (
    <div className="bg-white dark:bg-[#0c182b] border border-slate-200 dark:border-[#172a46] rounded-xl p-6 sm:p-8 shadow-xs transition-colors">
      ...
      {logs.length === 0 ? (
        <div className="text-center py-16 text-xs text-slate-400 dark:text-[#52719c] font-medium">
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="px-2 py-0.5 rounded text-[10px] font-mono font-semibold bg-[#0c83ff]/10 text-[#0c83ff] dark:text-[#3395ff] border border-[#0c83ff]/25">
                        {log.action_type}
                      </span>
                      {log.transaction_id && (
                        <span className="text-[10px] text-slate-400 dark:text-[#6a87aa] font-mono hidden md:inline">
                          {log.transaction_id}
                        </span>
                      )}
                    </div>

                    <div className="flex items-center gap-3 text-[11px] text-slate-500 dark:text-[#7a95b8] font-mono">
                      ...
                      <ChevronRight
                        className={`w-4 h-4 text-slate-400 transition-transform ${
                          isExpanded ? 'rotate-90 text-[#0c83ff]' : ''
                        }`}
                      />
                    </div>
                  </div>
                  ...
                  {/* Output Preview */}
                  {log.output_payload && (
                    <div className="mt-1.5 text-xs text-slate-600 dark:text-[#cad8ec] font-sans line-clamp-2">
                      {log.output_payload}
                    </div>
                  )}
