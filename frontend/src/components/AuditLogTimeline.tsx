  ...
  return (
    <div className="space-y-4 w-full">
      ...
      <div className="bg-white dark:bg-[#121215] border border-zinc-200 dark:border-[#27272a] rounded-lg p-4 sm:p-5 shadow-xs space-y-3.5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              aria-label="Search audit logs by Transaction ID, Agent, Action, or Payload"
              className="w-full h-9 pl-9 pr-9 bg-zinc-50 dark:bg-[#18181b] border border-zinc-200 dark:border-[#27272a] text-xs text-zinc-900 dark:text-white rounded-md font-mono focus:outline-none focus:border-blue-500 placeholder:text-zinc-400 placeholder:font-body focus-rzp"
            />
            {searchQuery && (
              <button
                type="button"
                ...
                className="absolute right-1 top-1 w-7 h-7 flex items-center justify-center text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 cursor-pointer rounded"
              >
                <X className="w-4 h-4" />
              </button>
            )}
          ...
          <div className="flex items-center gap-2 self-start sm:self-auto">
            <button
              ...
              aria-label={allExpanded ? 'Collapse all audit step cards' : 'Expand all audit step cards'}
              className="h-8 px-3 rounded-md bg-zinc-100 hover:bg-zinc-200 dark:bg-[#18181b] dark:hover:bg-[#27272a] text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-[#27272a] text-xs font-subheading font-medium inline-flex items-center gap-1.5 cursor-pointer disabled:opacity-50 transition-colors"
            >
            ...
            <div
              aria-live="polite"
              className="text-xs font-mono font-semibold px-2.5 py-1.5 rounded bg-zinc-100 dark:bg-[#18181b] text-zinc-800 dark:text-zinc-200 border border-zinc-200 dark:border-[#27272a]"
            >
        ...
        {/* Status Filter Chips */}
        <div role="group" aria-label="Filter audit logs by status" className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 pt-1 border-t border-zinc-100 dark:border-[#27272a]/60">
          {(
            [
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-subheading font-medium inline-flex items-center gap-1.5 whitespace-nowrap cursor-pointer transition-colors ${
      ...
      {logs.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredLogs.map((log) => {
            ...
            const statusIcon =
              log.status === 'success' ? (
                <div className="w-5 h-5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 flex items-center justify-center shrink-0" aria-label="Status: Success">
                  <CheckCircle2 className="w-3.5 h-3.5" />
                </div>
              ) : log.status === 'failure' ? (
                <div className="w-5 h-5 rounded-full bg-rose-500/10 border border-rose-500/20 text-rose-500 flex items-center justify-center shrink-0" aria-label="Status: Failure">
                  <AlertTriangle className="w-3.5 h-3.5" />
                </div>
              ) : (
                <div className="w-5 h-5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-500 flex items-center justify-center shrink-0" aria-label="Status: Skipped/Warning">
            ...
            return (
              <div
                key={log.id}
                className={`bg-white dark:bg-[#121215] border border-zinc-200 dark:border-[#27272a] rounded-lg shadow-xs overflow-hidden transition-colors ${
                  isExpanded ? 'ring-1 ring-blue-500/40 border-blue-500/50' : 'hover:border-zinc-300 dark:hover:border-zinc-700'
                }`}
              >
                ...
                <div
                  ...
                  className="p-4 cursor-pointer select-none space-y-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
                >
                ...
                {isExpanded && (
                  <div
                    ...
                    className="px-4 pb-4 pt-3 border-t border-zinc-200 dark:border-[#27272a] space-y-3 bg-zinc-50/50 dark:bg-[#09090b]/40"
                  >
